...
public class DeveloperSelectBannedStateRealNameFragment extends BaseFragment {
     ...
     * @return 单例对象
     */
    public static DeveloperSelectBannedStateRealNameFragment newInstance() {
        return new DeveloperSelectBannedStateRealNameFragment();
    }
    ...
    protected int bindLayout() {
        return R.layout.fragment_developer_select_banned_state_real_name;
    }
                ...
                .execute(new StringCallback() {
                    ...
                    public void onSuccess(Response<String> response) {
                        ...
                        if (401 == OkGoResponseBean.getCode() && "未提供Token".equals(OkGoResponseBean.getData()) && "验证失败，禁止访问".equals(OkGoResponseBean.getMsg())) {
                            Snackbar snackbar = Snackbar.make(mRlDevBanAccountStateRealName, "未登录：" + OkGoResponseBean.getMsg(), Snackbar.LENGTH_SHORT).setActionTextColor(getResources().getColor(R.color.colorAccent));
                            setSnackBarMessageTextColor(snackbar, Color.parseColor("#FFFFFF"));
                            snackbar.show();
                            return;
                        }
                        ...
                        //成功,已封禁
                        if (200 == OkGoResponseBean.getCode() && "此账户已封禁".equals(OkGoResponseBean.getMsg())) {
                            mEditBanAccountStateRealName.startShakeAnimation();//抖动输入框
