...
public class CanteenChaoYangFragment extends BaseFragment implements View.OnClickListener {
    ...
    private void startGetAllShopStoreInfo() {
        OkGo.<String>post(Constant.SELECT_ALL_SHOP_STORE_INFO_BY_SHOP_DESC + "/" + "朝阳校区")
                .tag("全部店铺信息")
    ...
    private void doResponseJsonData(Response<String> response) {
        ...
        if (200 == okGoAllShopInfoBean.getCode()&& "success".equals(okGoAllShopInfoBean.getMsg())){
            ...
            /* 商品信息信息列表适配器 */
            storeInfoAdapter = new BaseStoreInfoAdapter(R.layout.activity_canteen_recycler_view_item, storeInfoBeanList);
            //为列表控件设置适配器，并为执行适配操作
